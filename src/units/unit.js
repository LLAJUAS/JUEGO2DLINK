...
class Unit {
  ...
  step() {
    ...
    if (this.frameData.direction === 96) { // norteeee
      this.pos.y -= 1 * this.speed
    } else if (this.frameData.direction === 144) { // estee
      this.pos.x += 1 * this.speed
    } else if (this.frameData.direction === 0) { // sur
      this.pos.y += 1 * this.speed
    } else if (this.frameData.direction === 48) { // oeste
      this.pos.x -= 1 * this.speed
    }
  ...
  checkAvailableActions() {
    ...
    if (this.pos.row > 0 && this.grid[this.pos.row - 1][this.pos.col] === 1020) {
      neighbors.push([96, 0, -1]); // norte
    }
    if (this.grid[this.pos.row][this.pos.col + 1] === 1020) {
      neighbors.push([144, 1, 0]); // este
    }
    if (this.pos.row < 10 && this.grid[this.pos.row + 1][this.pos.col] === 1020) {
      neighbors.push([0, 0, 1]); // sur
    }
    if (this.grid[this.pos.row][this.pos.col - 1] === 1020) {
      neighbors.push([48, -1, 0]); // oesteee
    }
    return neighbors;
  }
